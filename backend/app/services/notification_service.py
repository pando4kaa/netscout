...
def _create_notifications_from_comparison(
    ...
    for ch in ssl.get("expired_changes") or []:
        if ch.get("is_expired_2") and not ch.get("was_expired_1"):
            notifications.append(Notification(
                ...
                title=f"Certificate expired: {ch.get('host', domain)}",
                message=f"SSL certificate for {ch.get('host', domain)} has expired.",
                details=ch,
                severity="critical",
            ))
    ...
    for h in ssl.get("hosts_only_in_2") or []:
        notifications.append(Notification(
            ...
            scan_id_prev=scan_id_prev,
            type="ssl_cert_new",
            title=f"New certificate: {h}",
            message=f"SSL certificate appeared for {h}.",
            details={"host": h},
            severity="info",
        ))
    ...
    if added:
        notifications.append(Notification(
            ...
            message=f"Added {len(added)} subdomain(s): {', '.join(added[:5])}{'...' if len(added) > 5 else ''}",
    ...
    if removed:
        notifications.append(Notification(
            ...
            message=f"Removed {len(removed)} subdomain(s): {', '.join(removed[:5])}{'...' if len(removed) > 5 else ''}",
    ...
    if new_alerts:
        notifications.append(Notification(
            ...
            message=f"Detected {len(new_alerts)} new alerts: {new_alerts[0].get('message', '')[:80]}...",
